...
class Test(TestCase):
    ...
    def test_at_gate_but_underleveled_print_message(self, mock_output):
        user_input = "2"
        character_stats = {"x_coordinate": 11, "player_level": 1, "y_coordinate": 8}
        game.level_one_region_restriction(user_input, character_stats)
    ...
    def test_at_gate_but_overleveled(self):
        user_input = "2"
        character_stats = {"x_coordinate": 11, "player_level": 2, "y_coordinate": 8}
        actual = game.level_one_region_restriction(user_input, character_stats)
        expected = None
        self.assertEqual(actual, expected)
    ...
    def test_at_wall_print_message(self, mock_output):
        user_input = "2"
        character_stats = {"x_coordinate": 9, "player_level": 1, "y_coordinate": 8}
        game.level_one_region_restriction(user_input, character_stats)
        the_function_printed_this = mock_output.getvalue()
